...
class AegisBot(commands.Bot):
    def __init__(self):
        ...
        self.polls:            Dict = {}   # {msg_id: poll_data}
        self.vc_pool:          Dict = {}
...
# Attrape TOUTES les erreurs de commandes slash → jamais "l'app ne répond plus"
@bot.tree.error
async def on_app_command_error(interaction: discord.Interaction,
                                error: app_commands.AppCommandError):
    msg = str(error)
    # Permissions manquantes → message clair
    if isinstance(error, app_commands.MissingPermissions):
        embed = er("Permission refusée",
                   f"Tu n'as pas la permission requise pour cette commande.\n`{msg}`")
    elif isinstance(error, app_commands.BotMissingPermissions):
        embed = er("Permission manquante (bot)",
                   f"Le bot n'a pas les permissions nécessaires.\n`{msg}`")
    elif isinstance(error, app_commands.CommandOnCooldown):
        embed = inf("Cooldown", f"Réessaie dans `{error.retry_after:.1f}s`.")
    else:
        logger.error(f"Slash command error [{interaction.command}]: {error}")
        embed = er("Erreur", f"Une erreur s'est produite.\n`{msg[:150]}`")
...
AI_SYS = ("Tu es Aegis, un bot Discord surpuissant et légèrement condescendant. "
          ...
          "Tu réponds TOUJOURS en français, 2-3 phrases max. "
          "Sarcastique subtil. Jamais vulgaire.")
...
async def play_next(gid: str):
    ...
    try:
        ...
        vol = discord.PCMVolumeTransformer(src, volume=0.5)
        vc.play(vol, after=lambda e: asyncio.run_coroutine_threadsafe(play_next(gid), bot.loop))
    except Exception as e:
...
async def check_spam(msg: discord.Message) -> bool:
    ...
    gid = str(msg.guild.id); uid = msg.author.id
    now = datetime.now(timezone.utc)
    cfg = bot.antispam_config.get(gid, {"enabled":True,"msg_limit":5,"msg_time":5,
    ...
    if spam:
        try:
            ...
            if   a=="kick": await msg.author.kick(reason=reason)
                ...
                await msg.author.timeout(datetime.now(timezone.utc)+timedelta(minutes=cfg.get("mute_duration",5)), reason=reason)
            await msg.channel.send(embed=mke(f"{T.SPAM}  Anti-Spam",
                                             f"{msg.author.mention} sanctionné\n**Raison :** {reason}",T.RED),
                                   delete_after=8)
...
class GiveawayView(discord.ui.View):
    def __init__(self, gid):
        super().__init__(timeout=None); self.add_item(GiveawayBtn(gid))
...
class PollBtn(discord.ui.Button):
    def __init__(self, poll_id: str, idx: int, label: str):
        super().__init__(label=label[:60],style=discord.ButtonStyle.secondary,
                         custom_id=f"poll_{poll_id}_{idx}",emoji=PEMOJIS[idx])
        self.poll_id=poll_id; self.idx=idx

    async def callback(self, interaction: discord.Interaction):
        # Chercher par ID du message Discord (fonctionne dans tous les serveurs)
        ...
        if votes.get(uid)==self.idx:
            del votes[uid]; msg=f"{T.CROSS} Vote retiré."
        else:
            votes[uid]=self.idx; msg=f"{T.CHECK} Voté pour **{poll['options'][self.idx]}** !"

        # Répondre IMMÉDIATEMENT (< 3s obligatoire)
        await interaction.response.send_message(msg,ephemeral=True)

        # Mettre à jour l'embed après la réponse
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(type=discord.ActivityType.watching,name="✨ /aide | Aegis V10"))
    try:
        await bot.application.edit(description=(
            "🤖 Aegis V10 — Bot Discord multifonction\n\n"
...
class TicketView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None); self.add_item(TicketBtn())
...
class RoleMenu(discord.ui.Select):
    def __init__(self,roles):
        ...
        super().__init__(placeholder="Choisis tes rôles...",min_values=0,max_values=len(opts),
                         options=opts,custom_id="role_select")
    async def callback(self,interaction: discord.Interaction):
...
class ReglementModal(discord.ui.Modal,title="✍️ Règlement personnalisé"):
    ...
    async def on_submit(self,interaction: discord.Interaction):
        ...
        e=discord.Embed(title=f"{T.SHIELD}  Règlement du serveur",description=self.contenu.value,
...
async def aide(interaction: discord.Interaction):
    e=discord.Embed(title=f"{T.SPARKLE}  Aegis V10 — Commandes",description="Toutes les commandes",
...
async def play(interaction: discord.Interaction,recherche: str):
    ...
    if not track or not track.get('url'):
        return await interaction.edit_original_response(embed=er("Introuvable","Aucun résultat."))
    vc=bot.vc_pool.get(gid)
    if not vc or not vc.is_connected():
        try: vc=await interaction.user.voice.channel.connect(); bot.vc_pool[gid]=vc
        except Exception as ex: return await interaction.edit_original_response(embed=er("Erreur vocal",str(ex)[:100]))
...
async def dmall(interaction: discord.Interaction,message: str):
    if not is_owner(interaction.user.id):
        return await interaction.response.send_message(
            embed=er("Accès refusé","Réservé au propriétaire du bot."),ephemeral=True)
    await interaction.response.defer(ephemeral=True)
    guild=interaction.guild; members=[m for m in guild.members if not m.bot]; total=len(members)
