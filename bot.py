...
# Cache anti-doublon pour les events membres
_processed_joins: set = set()
_processed_removes: set = set()

@bot.event
async def on_member_join(member:discord.Member):
    key = f"{member.guild.id}-{member.id}-{int(datetime.now(timezone.utc).timestamp()//10)}"
    if key in _processed_joins: return
    _processed_joins.add(key)
    if len(_processed_joins) > 200:
        try: _processed_joins.pop()
        except Exception: pass
...
async def on_member_remove(member:discord.Member):
    ...
    if len(_processed_removes) > 200:
        try: _processed_removes.pop()
        except Exception: pass
...
# Cache anti-doublon pour les messages (évite Railway overlap)
_processed_messages: set = set()

@bot.event
async def on_message(message:discord.Message):
    if message.author.bot: return
    # Guard anti-doublon (protection Railway double-instance)
    if message.id in _processed_messages: return
    _processed_messages.add(message.id)
    if len(_processed_messages) > 1000: # Nettoyer le cache
        try: _processed_messages.pop()
        except Exception: pass
    if message.guild:
